export async function createRaceBarChart(d3, targetElementId, width, height, margin, chartKey){
  let response;
  if(chartKey=="name"){
  ...
  // need to refactor, feeling sleepy for now :)
  // fetchData.forEach(data => {
  //   let years = Object.keys(data.overall);
  //   console.log("same ?: ",data[key]==data.name);
  //   years.forEach(year => {
  //     transformedData.push({
  //         name: key=="name"? data[key]:undefined,
  //         rank: data.rank,
  //         country: data.country,
  //         year: year,
  //         value: data.overall[year],
